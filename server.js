...
// Supports two modes:
//   A) JSON file: set FIREBASE_CREDENTIALS_PATH to the file path
//   B) Individual env vars: set each FIREBASE_* variable in Render
let firebaseApp;
try {
    let serviceAccount;

    if (process.env.FIREBASE_CREDENTIALS_PATH) {
        // Mode A — JSON file (local dev or Render Secret File)
        serviceAccount = require(process.env.FIREBASE_CREDENTIALS_PATH);
    } else {
        // Mode B — Individual environment variables (Render Environment panel)
        serviceAccount = {
