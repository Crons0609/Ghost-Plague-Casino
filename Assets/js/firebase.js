/**
 * ╔════════════════════════════════════════════════════════════════╗
 *  PHOENIX — firebase.js (VERSIÓN COMPAT - SIN MODULOS)
 *  Inicializa Firebase y expone Database + funciones CRUD al objeto window
...
window._obtenerSiguienteID = async function() {
  try {
    const snap = await Database.ref('Clientes').orderByChild('cliente_id').limitToLast(1).get();
...
window.guardarCliente = async function(cliente) {
  ...
  await Database.ref(`Clientes/${nextId}`).set({
    cliente_id: nextId,
    ...cliente,
    Estado:     'activo',
...
window.obtenerClientes = async function() {
  const snap = await Database.ref('Clientes').get();
...
window.escucharClientes = function(cb) {
  return Database.ref('Clientes').on('value', snap => {
...
window.actualizarCliente = async function(id, cambios) {
  await Database.ref(`Clientes/${id}`).update({
    ...cambios,
